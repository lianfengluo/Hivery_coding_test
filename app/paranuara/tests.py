...
class ApiTest(TestCase):
    ...
    def test_companies_employee(self):
      ...
      for i, company_id in enumerate(random_company_id_list):
        response = self.client.get(f'/api/paranuara/v1/company_employees/{company_id}/')
        response_people_indices = set(map(lambda x: x["id"], json.loads(response.content)))
        self.assertEqual(response_people_indices, set(employees_id_list[i]))
    ...
    def test_special_friends(self):
      ...
      for r_p1, r_p2 in random_people_pair_list:
        ...
        response = self.client.get(f'/api/paranuara/v1/special_common_friends/{r_id1}/{r_id2}/')
    ...
    def test_favourite_fruit(self):
      ...
      num_of_test = 10
      random_people_list = [random.choice(self.people) for _ in range(num_of_test)]
      for r_p in random_people_list:
        ...
        response = self.client.get(f'/api/paranuara/v1/food_info/{r_p["index"]}/')
        resp = json.loads(response.content)
